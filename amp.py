...
class AMP(Optimizer):
    ...
    def __init__(self, params, lr, epsilon, inner_lr=1, inner_iter=1, base_optimizer=SGD, **kwargs):
        if epsilon < 0.0:
            ...
            inner_iter = 0 # disable inner iteration if epsilon is zero
        if inner_lr < 0.0:
    ...
    def step(self, closure=None):
        ...
        outputs, loss = map(lambda x: x.detach(), closure()) # original loss and outputs
